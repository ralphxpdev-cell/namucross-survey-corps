...
/**
 * Survey Corps × Pi 런처 — ${member}님 전용
 * 생성: ${new Date().toLocaleString('ko-KR')}
 *
 * 실행: node sc-start.mjs
 */
...
pi.on('exit', async () => {
  ...
  try {
    ...
    if (saveRes.ok) {
      console.log(\`✅ 세션 저장 완료! 대시보드: \${SC_URL}/dashboard\`)
    } else {
      console.log('⚠️  세션 저장 실패 (' + saveRes.status + ')')
    }
  } catch (e) {
...
`
  ...
  return new NextResponse(script, {
    headers: {
      ...
      'Content-Disposition': `attachment; filename*=UTF-8''sc-start.mjs`,
    },
  })
}
